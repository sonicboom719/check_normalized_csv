...
- **階層制限**: `{prefecture}/{city}` または `{prefecture}/立候補者なし/{city}` の階層のみを処理（それ以下は対象外）
...
4. **階層的コピー**:
   - **第1階層（都道府県）**: コピー元フォルダ直下の都道府県フォルダを走査
   - **第2階層（市区町村または立候補者なし）**:
     - 通常の市区町村フォルダ: そのまま処理
     - 「立候補者なし」フォルダ: その下の市区町村フォルダを処理
   - **第3階層（市区町村内）**:
     - `*_normalized_final.csv` または `*_normalized_final_upd.csv`: 更新日時を比較してコピー
     - 「2025参院選後」フォルダ: 内部の `*_normalized_final_upd.csv` のみを親フォルダにコピー
     - その他のフォルダ/ファイル: スキップ
...
2026-01-18 10:00:06 INFO 都道府県数: 47
2026-01-18 10:00:07 INFO [1/47] 都道府県フォルダ: 愛媛県
2026-01-18 10:00:08 INFO   既存のフォルダを使用: 愛媛県 (ID: 1XyZ...)
2026-01-18 10:00:09 INFO   [1/47][1/20] 市区町村フォルダ: 愛南町
...
2026-01-18 10:00:13 INFO   [1/47][2/20] 市区町村フォルダ: 鬼北町
...
2026-01-18 10:00:16 INFO   [1/47][3/20] 市区町村フォルダ: 松野町
2026-01-18 10:00:17 INFO     既存のフォルダを使用: 松野町 (ID: 1Ghi...)
2026-01-18 10:00:18 INFO     スキップ（更新なし）: 松野町_normalized_final.csv (更新なし (コピー元: 2026-01-17T08:15:20.789Z, コピー先: 2026-01-18T11:45:30.123Z))
...
...
6. **階層制限**: `{prefecture}/{city}` または `{prefecture}/立候補者なし/{city}` の階層のみが処理対象。それより深い階層のフォルダ（「2025参院選後」を除く）は無視されます
